    ...
    for l=1:K
        ...
        for j=1:length(rand_sample) % for each swap
            ...
            if new_cost < prev_cost % if cost after swap is less than the previous cost
                ...
                convergence_factor = 0.2; % percentage of consecutive non-changes of medoids required for convergence
                if count >= round(convergence_factor*length(rand_sample)) % if the algorithm swaps through x% of the cluster's random sample without changing medoids
