...
% the kmeans matlab function directly. That is, you need to comment it out.
    ...
    %while n_iter < 10000
        for i=1:size(pixels,1) % for each data point
            ...
            for j=1:K % for each cluster
                for l=1:3 % compute distances in R, G, B
                    distances(j,l) = (centroids(j,l)-pixel(l)).^2;
                end
            end
            sum_dist = sum(distances, 2); % sum to get manhattan distance to each centroid
        ...
        for k=1:K % for each cluster
            ...
            centroids(k, :) = mean(part); % set new centroid
        end
